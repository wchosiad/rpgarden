...
import inquirer   # Used to show prompts (pip3 install inquirer)
...
try:
    ...
    calibratableSensorList = []
    for section in rpgConfig.parserInstance.sections():
        myType = rpgConfig.parserInstance.get(section, 'type', fallback=None)
        if myType and (myType == 'moisture' or myType == 'photo'):
            calibratableSensorList.append(section)
    
    print('===== Calibratable Sensors =====')
    for section in calibratableSensorList:
        print(section)
    
    calibratableSensorList.append("Quit")

    questions = [
        inquirer.List('sensor',
                        message="Which sensor do you wish to calibrate?",
                        choices=calibratableSensorList,
                    ),
    ]

    answers = inquirer.prompt(questions)

    print(answers["sensor"])
